...
// 请求只有 16 个所以可以直接暴力枚举所有的请求然后一一的来判断请求是否满足要求
class Solution {
private:
    vector<vector<int>> re;
    int cnt, ans, num, n;
    
    // 检查每一种组合是否满足，如果满足记录其最大满足的数量
    bool check(int st, int &num) 
    {
        vector<int> a(n, 0);
        for (int i = 0; i < cnt; i++)
        {
            // 用 st 表示组合是否有效 (一个状态压缩的方法)
            if (st & (1 << i))
            {
                a[re[i][1]]++, a[re[i][0]]--, num++;
            }
        }
        for (int i = 0; i < n; i++)
        {
...
public:
    int maximumRequests(int n, vector<vector<int>>& re) {
        ...
        for (int i = 0; i < (1 << cnt); i++)
        {
            ...
            // 每次如果组合满足，就更新最大满足的请求数量
            if (check(i, num))  ans = max(ans, num);
        }
